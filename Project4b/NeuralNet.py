...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """YOUR CODE"""
        newActs = [1.0] + inActs[:]
        return self.sigmoid(self.getWeightedSum(newActs))
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        # print "inacts", inActs
        newActs = [1.0] + inActs
        return self.sigmoidDeriv(self.getWeightedSum(newActs))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for (i, weight) in enumerate(self.weights):
            c = alpha * delta * newActs[i]
            self.weights[i] += c
            totalModification += abs(c)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            nextActs = []
            for n in layer:
                nextActs.append(n.sigmoidActivation(currActs))
            result.append(nextActs)
            currActs = nextActs
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            allLayerOutput = self.feedForward(example[0]) #output
            lastLayerOutput = allLayerOutput[-1] #actual
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                layer = self.hiddenLayers[layerNum]
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for j in range(len(nextLayer)):
                        delta += nextLayer[j].weights[neuronNum+1]*deltas[0][j]
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    testAccuracy = float(testCorrect) / (testError + testCorrect) #num correct/num total
