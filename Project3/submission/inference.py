...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: # If the ghost is captured
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        if noisyDistance == None: # a ghost has been captured
            temp = []
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        tmpPos = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        length = len(tmpPos)
        random.shuffle(tmpPos)

        self.particles = []
        for cnt in range(self.numParticles):
            self.particles.append(tmpPos[cnt % length])
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for g in range(self.numGhosts):
            if noisyDistances[g] == None: # captured
                captured.append(g)
        ...
        for p in self.particles:
            ...
            prob = 1.0
            for g in range(self.numGhosts):
                if g not in captured:
                    prob *= emissionModels[g][util.manhattanDistance(p[g], pacmanPosition)]
            tmpParticles[p] += prob
            if tmpParticles[p] != 0:
                ifAllZero = False
        ...
        for p in range(self.numParticles):
            self.particles[p] = util.sample(self.beliefs)
