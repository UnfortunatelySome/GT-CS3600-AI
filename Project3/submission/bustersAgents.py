...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for l in livingGhostPositionDistributions:
            likelyPos = l.argMax()
            dist = self.distancer.getDistance(pacmanPosition, likelyPos)
            if minDist == None or dist < minDist:
                minDist = dist
                likelyGhost = likelyPos
        # then choose an action that minimizes the distance to the closest ghost
        action = None
        minDist = None
        for l in legal:
            successorPos = Actions.getSuccessor(pacmanPosition, l)
            dist = self.distancer.getDistance(successorPos, likelyGhost)
            if minDist == None or dist < minDist:
                minDist = dist
                action = l
