...
class NQueensConstraint(BinaryConstraint):
	def __init__(self, var1, var2):
		...
		self.var2 = var2
		l = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']
		self.row1 = l.index(var1)
		self.row2 = l.index(var2)

	def isSatisfied(self, value1, value2):
		if value1 == value2:
			return False
		if math.fabs(int(value1)-int(value2)) == math.fabs(self.row1 - self.row2):
			return False
		return True

	def __repr__(self):
		return 'NQueensConstraint (%s, %s)' % (str(self.var1), str(self.var2))
...
def consistent(assignment, csp, var, value):
	...
	for c in csp.binaryConstraints:
		if c.affects(var):
			...
			if assignment.isAssigned(other) and not c.isSatisfied(value, assignment.assignedValues[other]):
				return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	...
	domains = orderValuesMethod(assignment, csp, var)
	for val in domains:
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for var, domain in domains.iteritems():
		if not assignment.isAssigned(var):
			# MRV
			if nextVar == None or len(domain) < len(domains[nextVar]):
				nextVar = var
			elif len(domain) == len(domains[nextVar]) and var != nextVar:
				# tie => degree heuristic
				bcons_var = [bcons for bcons in csp.binaryConstraints if bcons.affects(var)]
				bcons_nextVar = [bcons for bcons in csp.binaryConstraints if bcons.affects(nextVar)]
				if len(bcons_var) > len(bcons_nextVar):
					nextVar = var
	return nextVar
"""
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	"""Question 3"""
	"""YOUR CODE HERE"""
	# binary constraints of var
	bcons_var = [bcons for bcons in csp.binaryConstraints if bcons.affects(var)]
	# sort values by the LCV
	values.sort(cmp=lambda x,y: cmp(cValCnt(domains, bcons_var, var, x), cValCnt(domains, bcons_var, var, y)))
	return values

def cValCnt(domains, bcons_var, var, value):
	"""count the number of Constraining Values"""
	cvalues = 0
	for bc in bcons_var:
		neighbor = bc.otherVariable(var)
		removedValue = [r for r in domains[neighbor] if not bc.isSatisfied(value, r)]
		cvalues += len(removedValue)
	return cvalues
...
def revise(assignment, csp, var1, var2, constraint):
	...
	# """Question 5"""
	# """YOUR CODE HERE"""
	domains_var1 = list(assignment.varDomains[var1])
	domains_var2 = list(assignment.varDomains[var2])

	for val2 in domains_var2:
		satisfied = False
		for val1 in domains_var1:
			if constraint.isSatisfied(val1, val2):
				satisfied = True
				break
		if not satisfied:
			inferences.add((var2, val2))
			assignment.varDomains[var2].remove(val2)
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	while len(queue) > 0:
		var1, var2, c = queue.pop()
		tmp = revise(assignment, csp, var1, var2, c)
		if tmp == None:
			for variable, val in inferences:
				assignment.varDomains[variable].add(val)
			return None
		elif len(tmp) > 0:
			for b in csp.binaryConstraints: 
				if b.affects(var2) and assignment.assignedValues[b.otherVariable(var2)] == None:
					queue.append((var2, b.otherVariable(var2), b))
			inferences = inferences.union(tmp)
	return inferences
"""
